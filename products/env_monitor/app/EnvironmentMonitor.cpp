...
void EnvironmentMonitor::run_cli_task() {
    std::uint8_t received_character = 0;
    std::size_t  received_count     = 0;

    if (cli_uart_.read(&received_character, 1, received_count, 0) != uhal::Status::ok ||
        received_count == 0) {
        return;
    }
    ...
    if (command_length_ + 1 >= kCommandCapacity) {
        command_length_ = 0;
...
void EnvironmentMonitor::run_sensor_task() {
    ...
    led_level_           = temperature_celsius_ > 25 ? uhal::GpioLevel::high : uhal::GpioLevel::low;
